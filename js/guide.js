/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
   guide.js â€” Home page, Component Reference & Tutorials
...
/* â”€â”€ Pin type â†’ human readable label â”€â”€ */
...
const GUIDE_COMPONENTS = {
  ...
  arduino_uno: {
    ...
    pins: {
      'D0â€“D13': { label: 'D0â€“D13', type: 'digital', desc: 'Digital I/O pins. D0 (RX) and D1 (TX) double as the serial port. PWM capable pins are D3, D5, D6, D9, D10 and D11.' },
      'D13': { label: 'D13', type: 'digital', desc: 'Also drives the built-in "L" LED (LED_BUILTIN).' },
      'A0â€“A5': { label: 'A0â€“A5', type: 'analog', desc: 'Analog input pins (10-bit, 0â€“1023). A4 (SDA) and A5 (SCL) also serve the I2C bus.' },
      ...
      'VIN': { label: 'VIN', type: 'power', desc: 'Input voltage to the board (7â€“12 V via barrel jack).' },
      'GND': { label: 'GND', type: 'gnd', desc: 'Common ground â€” every component must share this reference.' },
      'AREF': { label: 'AREF', type: 'signal', desc: 'External analog reference voltage (advanced use).' },
      'RST': { label: 'RST', type: 'signal', desc: 'Reset line â€” pulling it low restarts the sketch.' },
    },
...
void loop() {
  ...
  esp32_devkit_v1: {
    ...
    pins: {
      'VP / VN': { label: 'VP / VN', type: 'analog', desc: 'Analog inputs 36 and 39 â€” no internal pull-up.' },
    ...
    props: { label: 'Board label shown on the canvas.' },
    wiring: 'Place the board and wire outputs to GPIO pins, sensors to ADC-capable pins (VP/VN/D32â€“D35). Remember to power modules from 3V3, not 5V.',
    code: `void setup() {
...
void loop() {
  ...
  arduino_nano: {
    ...
    pins: {
      'D0â€“D13': { label: 'D0â€“D13', type: 'digital', desc: 'Digital I/O pins. D0 (RX) and D1 (TX) double as the serial port. PWM capable pins are D3, D5, D6, D9, D10 and D11.' },
      'D13': { label: 'D13', type: 'digital', desc: 'Also drives the built-in "L" LED (LED_BUILTIN).' },
      'A0â€“A5': { label: 'A0â€“A5', type: 'analog', desc: 'Analog input pins (10-bit, 0â€“1023).' },
      '5V': { label: '5V', type: 'power', desc: '5 V regulated output for powering external components.' },
...
void loop() {
  ...
  stm32f746_disco: {
    ...
    pins: {
      'D0â€“D13': { label: 'D0â€“D13', type: 'digital', desc: 'Digital I/O pins. D0 (PC7) is USART6_RX, D1 (PC6) is USART6_TX. D3 (PB4) is PWM (TIM3_CH1). D10 (PA8) is SPI2_NSS. D13 (PI1) drives LD1 (user LED).' },
      ...
      'A0â€“A5': { label: 'A0â€“A5', type: 'analog', desc: 'Analog input pins (12-bit ADC, 0â€“4095). A0=PA0 (ADC3_IN0), A1=PF10, A2=PF9, A3=PF8, A4=PF7, A5=PF6.' },
      '5V': { label: '5V', type: 'power', desc: '5V power rail from USB or VIN.' },
      '3V3': { label: '3.3V', type: 'power', desc: '3.3V regulated output. All GPIOs are 3.3V logic â€” do NOT connect 5V signals directly.' },
      'GND': { label: 'GND', type: 'gnd', desc: 'Common ground.' },
    ...
    props: { label: 'Board label shown on the canvas.' },
    wiring: 'Connect outputs to digital pins D0â€“D15, sensors to analog pins A0â€“A5. Use 3.3V for powering modules. D14/D15 provide I2C (SDA/SCL). D10â€“D13 provide SPI (NSS/MOSI/MISO/SCK).',
    code: `void setup() {
...
void loop() {
  ...
  lpc2148: {
    ...
    use: 'An educational ARM7 development board for learning embedded systems, register-level GPIO control, ADC/DAC experiments, serial communication, and timer interrupts.',
    pins: {
      'P0.0-P0.13': { label: 'P0.0-P0.13', type: 'digital', desc: 'GPIO Port 0 lower pins. P0.2/P0.3 = UART0 TXD/RXD. P0.4/P0.5 = UART1 TXD/RXD. P0.7-P0.10 = SPI0. P0.12/P0.13 = onboard LED1/LED2.' },
      'P0.14-P0.28': { label: 'P0.14-P0.28', type: 'digital', desc: 'GPIO Port 0 upper pins. P0.14/P0.15 = onboard LED3/LED4. P0.25 = DAC. P0.26-P0.28 = ADC AIN0-AIN2.' },
      'VCC': { label: 'VCC (3.3V)', type: 'power', desc: '3.3V regulated output. All GPIOs are 3.3V logic.' },
      'GND': { label: 'GND', type: 'gnd', desc: 'Common ground.' },
      'VBUS': { label: 'VBUS (5V)', type: 'power', desc: '5V from USB connection.' },
      'RST': { label: 'RST', type: 'signal', desc: 'MCU reset (active low).' },
    },
    props: { label: 'Board label shown on the canvas.' },
    wiring: 'Use P0.12-P0.15 for onboard LEDs (no wiring needed). Connect external components to GPIO pins. All logic is 3.3V.',
  ...
  led: {
    ...
    icon: 'ðŸ’¡',
    category: 'Output',
    longDesc: 'A Light Emitting Diode that glows when current flows from the anode to the cathode. An LED has polarity â€” the longer leg is the anode (+). Always use a current-limiting resistor (typically 220 Î©) in series so the LED is not destroyed.',
    ...
    pins: {
      anode: { label: '+', type: 'pwm', desc: 'Anode (long leg). Connect to a digital pin through a resistor. Accepts HIGH/LOW or PWM (0â€“255).' },
      cathode: { label: 'âˆ’', type: 'gnd', desc: 'Cathode (short leg, flat side). Connect to GND.' },
    },
    props: {
      color: 'Hex colour of the LED body (e.g. #ff3333).',
      colorName: 'Human readable colour name.',
    },
    wiring: 'D13 â†’ resistor â†’ LED anode(+) ; LED cathode(âˆ’) â†’ GND.',
    code: `void setup() {
  pinMode(13, OUTPUT);
}
void loop() {
  ...
  multi_led_array: {
    ...
    pins: {
      ...
      gnd: { label: 'âˆ’', type: 'gnd', desc: 'Common cathode â€” connect to GND.' },
    },
    props: {},
    wiring: 'R â†’ D10, Y â†’ D11, G â†’ D12, B â†’ D13 ; GND â†’ GND.',
    code: `void setup() {
...
void loop() {
  ...
  rgb_led: {
    ...
    icon: 'ðŸŒˆ',
    category: 'Output',
    longDesc: 'A single LED package containing a red, green and blue die. By mixing the three channels with PWM you can produce almost any colour. This variant is a common-cathode LED â€” the shared pin goes to ground.',
    use: 'Full-colour lighting, mood lamps, colour-mixing demos. Each channel is driven from a PWM pin via analogWrite().',
    pins: {
      red: { label: 'R', type: 'pwm', desc: 'Red channel anode â€” PWM (0â€“255).' },
      green: { label: 'G', type: 'pwm', desc: 'Green channel anode â€” PWM (0â€“255).' },
      blue: { label: 'B', type: 'pwm', desc: 'Blue channel anode â€” PWM (0â€“255).' },
      gnd: { label: 'âˆ’', type: 'gnd', desc: 'Common cathode â€” connect to GND.' },
    },
    props: {},
    wiring: 'R â†’ D9, G â†’ D10, B â†’ D11 (PWM pins) ; GND â†’ GND.',
    code: `void setup() {
...
void loop() {
  ...
  buzzer: {
    ...
    pins: {
      vcc: { label: '+', type: 'digital', desc: 'Positive supply â€” connect to a digital pin (HIGH beeps).' },
      gnd: { label: 'âˆ’', type: 'gnd', desc: 'Negative supply â€” connect to GND.' },
    },
    props: { frequency: 'Default tone frequency in Hz when driven by tone().' },
    wiring: 'D8 â†’ buzzer(+) ; buzzer(âˆ’) â†’ GND.',
    code: `void setup() {
  pinMode(8, OUTPUT);
}
void loop() {
  ...
  seg7: {
    ...
    icon: 'ðŸ”¢',
    category: 'Output',
    longDesc: 'A single digit made from seven LED bars (aâ€“g) plus a decimal point. Each segment is a separate LED; lighting the right combination shows any digit 0â€“9 or some letters. The common pin (COM) completes the circuit â€” set commonAnode false for a common-cathode module.',
    ...
    pins: {
      ...
      com: { label: 'COM', type: 'power', desc: 'Common pin â€” connect to GND (common cathode) or 5V (common anode).' },
    },
    props: { commonAnode: 'true if the display is common-anode (COM to +5V).' },
    wiring: 'Aâ†’D2, Bâ†’D3, Câ†’D4, Dâ†’D5, Eâ†’D6, Fâ†’D7, Gâ†’D8 ; COM â†’ GND.',
...
void loop(){
  ...
  lcd1602: {
    ...
    icon: 'ðŸ–¥ï¸',
    category: 'Output',
    longDesc: 'A classic 16-column Ã— 2-row character LCD driven by the Hitachi HD44780 controller. Used through the LiquidCrystal library, it needs 6 control/data lines (RS, EN, D4â€“D7) plus power. Perfect for text, sensor readouts and simple menus.',
    ...
    pins: {
      ...
      rs: { label: 'RS', type: 'digital', desc: 'Register select â€” command vs. data.' },
      rw: { label: 'R/W', type: 'digital', desc: 'Read/Write â€” tie to GND for write-only.' },
      en: { label: 'EN', type: 'digital', desc: 'Enable pulse â€” latches the data.' },
    ...
    wiring: 'VCCâ†’5V, GNDâ†’GND, RSâ†’D12, ENâ†’D11, D4â†’D5, D5â†’D4, D6â†’D3, D7â†’D2.',
...
void loop(){
  ...
  lcd1602_i2c: {
    ...
    icon: 'ðŸ–¥ï¸',
    category: 'Output',
    longDesc: 'The same 16Ã—2 character LCD but with a PCF8574 I2C backpack â€” only four wires are needed (GND, VCC, SDA, SCL) and the default address is 0x27. Use the LiquidCrystal_I2C library.',
    use: 'Text and data displays with minimal wiring â€” ideal when pins are scarce. Uses A4 (SDA) and A5 (SCL) on the Uno.',
    pins: {
      gnd: { label: 'GND', type: 'gnd', desc: 'Ground.' },
      vcc: { label: 'VCC', type: 'power', desc: '5 V power.' },
      sda: { label: 'SDA', type: 'digital', desc: 'I2C data line â€” connect to A4 (Uno) / D21 (ESP32).' },
      scl: { label: 'SCL', type: 'digital', desc: 'I2C clock line â€” connect to A5 (Uno) / D22 (ESP32).' },
    },
    props: {
    ...
    wiring: 'VCCâ†’5V, GNDâ†’GND, SDAâ†’A4, SCLâ†’A5.',
...
void loop(){
  ...
  oled_ssd1306: {
    id: 'oled_ssd1306',
    name: 'OLED 128Ã—64 (I2C)',
    icon: 'ðŸ–¥ï¸',
    category: 'Output',
    longDesc: 'A 128Ã—64 monochrome OLED screen driven by the SSD1306 controller over I2C. Sharp, fast and low-power. Driven with the Adafruit_SSD1306 + Adafruit_GFX libraries, it can render text, lines, circles, rectangles and even pixel art.',
    ...
    pins: {
      ...
      scl: { label: 'SCL', type: 'digital', desc: 'I2C clock â€” connect to A5 (Uno) / D22 (ESP32).' },
      sda: { label: 'SDA', type: 'digital', desc: 'I2C data â€” connect to A4 (Uno) / D21 (ESP32).' },
    },
...
void loop(){}`,
  ...
  push_button: {
    ...
    icon: 'ðŸ”˜',
    category: 'Input',
    longDesc: 'A momentary tactile switch. The four legs form two internally-connected pairs â€” pressing the cap joins the pairs so current can flow. Read it with digitalRead() and an INPUT_PULLUP so the pin reads HIGH when released and LOW when pressed. Works standalone with ICs (no Arduino needed) â€” the simulator traces through the switch to find the voltage source on the other side.',
    use: 'User input â€” buttons, doorbells, triggers. Wire one side to a digital pin (with internal pull-up) and the other to GND. Toggle it by clicking on the canvas or pressing Space/Enter when selected.',
    pins: {
      p1: { label: '1', type: 'digital', desc: 'Leg 1 â€” one side of the switch contact.' },
      p2: { label: '2', type: 'digital', desc: 'Leg 2 â€” internally connected to leg 1.' },
      p3: { label: '3', type: 'digital', desc: 'Leg 3 â€” the other side of the switch contact.' },
      p4: { label: '4', type: 'digital', desc: 'Leg 4 â€” internally connected to leg 3.' },
    },
    props: {
      pressed: 'Simulated pressed state when toggled in the canvas.',
      label: 'Button label shown on the canvas.',
    },
    wiring: 'D2 â†’ pin 1, pin 3 â†’ GND. Enable INPUT_PULLUP in code.',
    code: `void setup(){
...
void loop(){
  ...
  potentiometer: {
    ...
    pins: {
      ...
      wiper: { label: 'OUT', type: 'analog', desc: 'Variable output â€” connect to an analog input (A0â€“A5).' },
      gnd: { label: 'GND', type: 'gnd', desc: 'Connect to GND.' },
    },
    props: {
      value: 'Current wiper position (0â€“1023).',
      maxValue: 'Full-scale value (default 1023).',
      resistance: 'Resistance in ohms (default 10000).',
    },
    wiring: 'VCCâ†’5V, OUTâ†’A0, GNDâ†’GND.',
    code: `void setup(){
  Serial.begin(9600);
}
void loop(){
  ...
  joystick: {
    ...
    icon: 'ðŸ•¹ï¸',
    category: 'Input',
    longDesc: 'A two-axis analog joystick with a push button (SW). Each axis is a pair of potentiometers giving a 0â€“1023 value (roughly 512 at centre), and the button reads as a digital input (LOW when pressed).',
    use: 'Game controllers, robotics, menu navigation. Read X and Y on two analog pins and the button on a digital pin.',
    pins: {
      gnd: { label: 'GND', type: 'gnd', desc: 'Ground.' },
      vcc: { label: 'VCC', type: 'power', desc: '5 V power.' },
      x: { label: 'X', type: 'analog', desc: 'X-axis analog output â€” connect to A0.' },
      y: { label: 'Y', type: 'analog', desc: 'Y-axis analog output â€” connect to A1.' },
      sw: { label: 'SW', type: 'digital', desc: 'Push button output (LOW when pressed).' },
    },
    props: {
      x: 'X-axis position (0â€“1023).',
      y: 'Y-axis position (0â€“1023).',
      sw: 'Button state (0 = released, 1 = pressed).',
    },
    wiring: 'VCCâ†’5V, GNDâ†’GND, Xâ†’A0, Yâ†’A1, SWâ†’D2.',
    code: `void setup(){
  pinMode(2, INPUT_PULLUP);
  Serial.begin(9600);
}
...
void loop() {
  ...
  servo: {
    ...
    icon: 'âš™ï¸',
    category: 'Actuators',
    longDesc: 'An RC servo motor that rotates to a precise angle between 0Â° and 180Â°. Driven by a 50 Hz PWM signal whose pulse width encodes the target angle â€” the Servo library does this for you with servo.write(angle).',
    use: 'Robotics, pan/tilt heads, gauges, automation. One signal wire + power and ground is all it needs.',
    pins: {
      signal: { label: 'SIG', type: 'pwm', desc: 'Control signal â€” connect to a PWM-capable pin (D9/D10 on Uno).' },
      vcc: { label: '+', type: 'power', desc: 'Power â€” 5 V.' },
      gnd: { label: 'âˆ’', type: 'gnd', desc: 'Ground.' },
    },
    props: {
      angle: 'Target angle (0â€“180Â°).',
      minAngle: 'Minimum angle (default 0).',
      maxAngle: 'Maximum angle (default 180).',
    },
    wiring: 'SIGâ†’D9, +â†’5V, âˆ’â†’GND.',
...
void loop(){
  ...
  dc_motor: {
    ...
    icon: 'ðŸŒ€',
    category: 'Actuators',
    longDesc: 'A brushed DC motor whose speed follows the PWM duty cycle applied to its input pin â€” 0 stops it, 255 is full speed. The shaft and fan spin proportionally to the PWM value so you can see speed at a glance.',
    ...
    pins: {
      in: { label: 'IN', type: 'pwm', desc: 'Speed input â€” PWM (0â€“255).' },
      gnd: { label: 'GND', type: 'gnd', desc: 'Ground.' },
    },
    props: { label: 'Motor label shown on the canvas.' },
    wiring: 'INâ†’D9 (PWM), GNDâ†’GND.',
    code: `void setup(){
  pinMode(9, OUTPUT);
}
void loop(){
  ...
  relay: {
    ...
    pins: {
      vcc: { label: 'VCC', type: 'power', desc: 'Module power â€” 5 V.' },
      gnd: { label: 'GND', type: 'gnd', desc: 'Ground.' },
      sig: { label: 'IN', type: 'digital', desc: 'Control signal â€” drives the coil.' },
      com: { label: 'COM', type: 'signal', desc: 'Common contact of the switched circuit.' },
      no: { label: 'NO', type: 'signal', desc: 'Normally-open contact â€” connects to COM when energised.' },
      nc: { label: 'NC', type: 'signal', desc: 'Normally-closed contact â€” connected to COM when idle.' },
    },
    props: { label: 'Relay label shown on the canvas.' },
    wiring: 'VCCâ†’5V, GNDâ†’GND, INâ†’D9, 5Vâ†’COM, NOâ†’LED anode.',
    code: `void setup(){
  pinMode(9, OUTPUT);
}
void loop(){
  digitalWrite(9, HIGH);   // relay on, COMâ†’NO
  delay(1000);
  digitalWrite(9, LOW);    // relay off, COMâ†’NC
  delay(1000);
}`,
  ...
  dht11: {
    ...
    icon: 'ðŸŒ¡ï¸',
    category: 'Sensors',
    longDesc: 'A digital temperature and humidity sensor. It reports 0â€“50 Â°C temperature and 20â€“90 % relative humidity over a single data wire using a custom one-wire protocol (the DHT library handles it). Adjust the simulated values live from the property panel.',
    ...
    pins: {
      vcc: { label: 'VCC', type: 'power', desc: '3.3 V / 5 V power.' },
      data: { label: 'DAT', type: 'digital', desc: 'One-wire data signal â€” connect to a digital pin.' },
      nc: { label: 'NC', type: 'signal', desc: 'Not connected.' },
      gnd: { label: 'GND', type: 'gnd', desc: 'Ground.' },
    },
    props: {
      temperature: 'Simulated temperature in Â°C (0â€“50).',
      humidity: 'Simulated relative humidity in % (20â€“90).',
    },
    wiring: 'VCCâ†’5V, DATâ†’D2, GNDâ†’GND.',
    code: `#include <DHT.h>
DHT dht(2, DHT11);
void setup(){
  Serial.begin(9600);
...
void loop(){
  ...
  hcsr04: {
    ...
    icon: 'ðŸ“¡',
    category: 'Sensors',
    longDesc: 'An ultrasonic distance sensor that measures 2â€“400 cm. Send a 10 Âµs HIGH pulse on TRIG; the ECHO pin then stays HIGH for a duration proportional to the distance. Distance (cm) = echoTime(Âµs) / 58.',
    ...
    pins: {
      vcc: { label: 'VCC', type: 'power', desc: '5 V power.' },
      trig: { label: 'TRIG', type: 'digital', desc: 'Trigger â€” send a 10 Âµs HIGH pulse to start a measurement.' },
      echo: { label: 'ECHO', type: 'digital', desc: 'Echo â€” returns HIGH for the flight time of the ping.' },
      gnd: { label: 'GND', type: 'gnd', desc: 'Ground.' },
    },
    props: { distance: 'Simulated object distance in cm (2â€“400).' },
    wiring: 'VCCâ†’5V, TRIGâ†’D7, ECHOâ†’D8, GNDâ†’GND.',
    code: `void setup(){
...
void loop(){
  ...
  ldr: {
    ...
    icon: 'ðŸ’¡',
    category: 'Sensors',
    longDesc: 'A light-dependent resistor whose resistance falls as light increases. Wired as a voltage divider it produces a 0â€“1023 analog value â€” low in bright light, high in darkness. Change the light level live from the property panel.',
    use: 'Automatic night lights, light meters, camera exposure. Read the light level on an analog pin.',
    pins: {
      vcc: { label: 'VCC', type: 'power', desc: '5 V power.' },
      a: { label: 'A', type: 'analog', desc: 'Analog output (0â€“1023) â€” connect to A0.' },
      gnd: { label: 'GND', type: 'gnd', desc: 'Ground.' },
    },
    props: { light: 'Simulated light level (0â€“1023, higher = brighter).' },
    wiring: 'VCCâ†’5V, Aâ†’A0, GNDâ†’GND.',
    code: `void setup(){
  Serial.begin(9600);
}
void loop(){
  ...
  pir: {
    ...
    pins: {
      vcc: { label: 'VCC', type: 'power', desc: '5 V power.' },
      out: { label: 'OUT', type: 'digital', desc: 'Motion output â€” HIGH while motion is detected.' },
      gnd: { label: 'GND', type: 'gnd', desc: 'Ground.' },
    },
    props: { motion: 'Simulated motion (0 = idle, 1 = motion detected).' },
    wiring: 'VCCâ†’5V, OUTâ†’D2, GNDâ†’GND.',
    code: `void setup(){
  pinMode(2, INPUT);
  pinMode(13, OUTPUT);
  Serial.begin(9600);
...
void loop(){
  ...
  lm35_sensor: {
    ...
    pins: {
      VCC: { label: 'VCC', type: 'power', desc: 'Power supply â€” 5 V.' },
      OUT: { label: 'OUT', type: 'analog', desc: 'Analog voltage output (10 mV/Â°C) â€” connect to A0â€“A5.' },
      GND: { label: 'GND', type: 'gnd', desc: 'Ground.' },
    },
    props: {
...
void loop() {
  ...
  resistor: {
    ...
    icon: 'â¬›',
    category: 'Passive',
    longDesc: 'A two-terminal component that limits current. Its value in ohms is shown by the colour bands and adjustable in the property panel. A 220 Î© resistor in series with an LED is the classic current limiter.',
    ...
    wiring: 'In series anywhere in a circuit â€” polarity does not matter.',
...
// Example: LED anode -> resistor -> 5V limits LED current.`,
  ...
  capacitor: {
    ...
    pins: {
      ...
      neg: { label: 'âˆ’', type: 'gnd', desc: 'Negative terminal.' },
    },
    props: { value: 'Capacitance in ÂµF (e.g. 100).' },
    wiring: 'Across the power rails: + to 5V and âˆ’ to GND to smooth noise.',
  ...
  breadboard: {
    ...
    icon: 'ðŸŸ¦',
    category: 'Passive',
    longDesc: 'A solderless prototyping board. The two long power rails on the edges carry + and âˆ’ along the whole strip; the middle rows connect holes vertically. Use it to build neat, expandable circuits.',
    use: 'Breadboarding temporary circuits â€” connect power rails to the Arduino and plug LEDs, buttons and sensors into the middle rows.',
    pins: {},
    props: {},
    wiring: 'Wire the + rail to 5V and the âˆ’ rail to GND, then build your circuit in the middle rows.',
  ...
  power_gnd: {
    ...
    icon: 'âš',
    category: 'Power',
    longDesc: 'A standalone ground terminal. Every circuit needs a common ground reference â€” use this to give your components a clean ground rail.',
  ...
  multimeter: {
    ...
    longDesc: 'A digital multimeter that measures DC/AC voltage, resistance, continuity, and DC current between two probe points. Select the mode from the properties panel â€” the LCD display updates live during simulation.',
    ...
    pins: {
      probe_red: { label: 'V+', type: 'signal', desc: 'Red probe â€” positive measurement terminal.' },
      probe_com: { label: 'COM', type: 'gnd', desc: 'Black probe â€” common / ground terminal.' },
    },
    props: {
    ...
    code: `// The multimeter is a measuring instrument â€” no Arduino code needed.`,
  ...
  func_gen: {
    ...
    use: 'Signal source for testing circuits â€” drive LEDs, buzzers, servos, or logic circuits with configurable waveforms. CH1 and CH2 outputs are independent voltage sources.',
    pins: {
    ...
    props: {
      ...
      ch1_freq: 'CH1 frequency in Hz (10â€“3000).',
      ch1_amp: 'CH1 peak-to-peak amplitude in volts (0.1â€“10).',
      ch1_offset: 'CH1 DC offset in volts (-5 to +5).',
      ch1_phase: 'CH1 phase in degrees (0â€“360).',
      ch1_duty: 'CH1 duty cycle in percent (5â€“95).',
      ch2_wave: 'CH2 waveform: sine, square, triangle, sawtooth, noise.',
      ch2_freq: 'CH2 frequency in Hz (10â€“3000).',
      ch2_amp: 'CH2 peak-to-peak amplitude in volts (0.1â€“10).',
      ch2_offset: 'CH2 DC offset in volts (-5 to +5).',
      ch2_phase: 'CH2 phase in degrees (0â€“360).',
      ch2_duty: 'CH2 duty cycle in percent (5â€“95).',
    },
    wiring: 'Connect CH1 or CH2 output to your circuit input. Connect the corresponding GND to the circuit ground.',
    code: `// The function generator is a signal source â€” no Arduino code needed.`,
    exampleId: 'func_gen_led',
  },

  /* â”€â”€ NEOPIXEL EXTENSIONS â”€â”€ */
  neopixel_strip: {
    id: 'neopixel_strip',
    ...
    pins: {
      ...
      DIN: { label: 'DIN', type: 'digital', desc: 'Data input â€” connect to an Arduino digital pin (e.g. D6).' },
      GND: { label: 'GND', type: 'gnd', desc: 'Common ground.' },
      DOUT: { label: 'DOut', type: 'digital', desc: 'Data output â€” chain to the DIN of the next strip.' },
    },
    props: {
      numPixels: 'Number of pixels (default 8).',
      brightness: 'Global brightness 0â€“255.',
    },
    wiring: 'VCC â†’ 5 V, GND â†’ GND, DIN â†’ Arduino D6 (or any digital pin). Use a 330â€“470 Î© resistor on the data line and a 1000 ÂµF capacitor across VCC/GND for best results.',
...
void loop() {
  ...
  neopixel_ring: {
    ...
    use: 'Building radial animations, rotary indicators, or circular colour effects. Works identically to the NeoPixel Strip in code â€” just a different physical layout.',
    pins: {
      VCC: { label: 'VCC', type: 'power', desc: '5 V power supply.' },
      DIN: { label: 'DIN', type: 'digital', desc: 'Data input â€” connect to an Arduino digital pin.' },
      DOUT: { label: 'DOut', type: 'digital', desc: 'Data output â€” chain to the next NeoPixel device.' },
      GND: { label: 'GND', type: 'gnd', desc: 'Common ground.' },
    },
    props: {
    ...
    wiring: 'VCC â†’ 5 V, GND â†’ GND, DIN â†’ Arduino D6. Same decoupling recommendations as the strip (1000 ÂµF cap + 470 Î© resistor on DIN).',
    code: `#include <Adafruit_NeoPixel.h>
...
void loop() {
  ...
  ic_555: {
    ...
    icon: 'â±ï¸',
    category: 'ICs',
    longDesc: 'The classic 555 timer IC configured in astable mode. The simulator automatically detects connected R1, R2, and C values and calculates the real oscillation frequency using the standard formulas: f = 1.44 / ((R1 + 2Ã—R2) Ã— C). Works standalone without an Arduino â€” the OUT pin drives LEDs and other components directly.',
    ...
    pins: {
      ...
      TRIG: { label: 'TRIG', type: 'digital', desc: 'Trigger input â€” starts the cycle when voltage falls below 1/3 VCC.' },
      OUT: { label: 'OUT', type: 'digital', desc: 'Output pin â€” drives HIGH (~VCC) or LOW (GND).' },
      RST: { label: 'RST', type: 'digital', desc: 'Reset â€” tie to VCC to keep timer active.' },
      DIS: { label: 'DIS', type: 'digital', desc: 'Discharge pin â€” internal transistor sinks current to GND when OUT is LOW.' },
      THR: { label: 'THR', type: 'digital', desc: 'Threshold input â€” resets the cycle when voltage exceeds 2/3 VCC.' },
      CV: { label: 'CV', type: 'signal', desc: 'Control voltage â€” add a 0.1 ÂµF cap to GND for noise bypass.' },
      VCC: { label: 'VCC', type: 'power', desc: 'Supply voltage (4.5â€“16 V).' },
    },
    props: {
    ...
    wiring: 'VCCâ†’5V, GNDâ†’GND, RSTâ†’VCC, CVâ†’0.1ÂµFâ†’GND, OUTâ†’LED+resistorâ†’GND. For astable: VCCâ†’R1â†’DIS, DISâ†’R2â†’THR, THRâ†’Câ†’GND, TRIGâ†’THR.',
    code: `// The 555 timer runs autonomously â€” no Arduino code needed.
...
//   f = 1.44 / ((R1 + 2*R2) * C)`,
  ...
  max7219: {
    ...
    icon: 'ðŸ”´',
    category: 'ICs',
    longDesc: 'An 8Ã—8 LED dot matrix driver using the MAX7219 chip with SPI interface. Supports cascading via DOUTâ†’DIN. The simulator decodes SPI commands and renders the LED matrix on the canvas. Works with or without an Arduino.',
    use: 'Scrolling text displays, animations, bar graphs. Driven via SPI with DIN, CS, and CLK pins.',
    pins: {
      VCC: { label: 'VCC', type: 'power', desc: '5 V power supply.' },
      GND: { label: 'GND', type: 'gnd', desc: 'Ground.' },
      DIN: { label: 'DIN', type: 'digital', desc: 'SPI data input â€” connect to MOSI or any digital pin.' },
      CS: { label: 'CS', type: 'digital', desc: 'Chip select â€” active LOW. Latch data on rising edge.' },
      CLK: { label: 'CLK', type: 'digital', desc: 'SPI clock input.' },
      DOUT: { label: 'DOUT', type: 'digital', desc: 'Data output â€” chain to the next MAX7219 DIN.' },
    },
    props: {},
    wiring: 'VCCâ†’5V, GNDâ†’GND, DINâ†’D11 (MOSI), CSâ†’D10, CLKâ†’D13 (SCK). For cascading: DOUTâ†’next DIN.',
    code: `// Uses SPI to send 16-bit frames: [address][data]
// Address 0x01â€“0x08: row data (bit 0 = left, bit 7 = right)
// Address 0x09: decode mode (0x00 = no decode)
// Address 0x0A: intensity (0x00â€“0x0F)
...
// Address 0x0C: shutdown (0x01 = normal operation)`,
  ...
  ili9341: {
    ...
    icon: 'ðŸ–¥ï¸',
    category: 'Output',
    longDesc: 'A 2.4â€³ 320Ã—240 TFT LCD driven by the ILI9341 controller with SPI interface. Supports Adafruit_GFX drawing primitives â€” lines, rectangles, circles, triangles, text, and pixel-level control. The simulator renders a 5Ã—7 ASCII font for text and colour RGB drawing primitives.',
    use: 'Colour graphics, dashboards, games, data visualization. Uses SPI: CS, DC, RST, MOSI, SCK, and LED backlight pins.',
    pins: {
      VCC: { label: 'VCC', type: 'power', desc: '3.3 V / 5 V power.' },
      GND: { label: 'GND', type: 'gnd', desc: 'Ground.' },
      CS: { label: 'CS', type: 'digital', desc: 'Chip select â€” active LOW.' },
      DC: { label: 'DC', type: 'digital', desc: 'Data/Command select â€” HIGH for data, LOW for command.' },
      RST: { label: 'RST', type: 'digital', desc: 'Hardware reset â€” active LOW.' },
      MOSI: { label: 'MOSI', type: 'digital', desc: 'SPI master-out data.' },
      SCK: { label: 'SCK', type: 'digital', desc: 'SPI clock.' },
      LED: { label: 'LED', type: 'power', desc: 'Backlight control â€” tie to VCC for always-on.' },
    },
    props: {},
    wiring: 'VCCâ†’3.3V/5V, GNDâ†’GND, CSâ†’D10, DCâ†’D9, RSTâ†’D8, MOSIâ†’D11, SCKâ†’D13, LEDâ†’VCC.',
    code: `#include <SPI.h>
#include <Adafruit_GFX.h>
...
void loop() {}`,
  ...
  ir_obstacle: {
    ...
    pins: {
      VCC: { label: 'VCC', type: 'power', desc: '3.3 V / 5 V power.' },
      GND: { label: 'GND', type: 'gnd', desc: 'Ground.' },
      OUT: { label: 'OUT', type: 'digital', desc: 'Digital output â€” LOW when obstacle detected, HIGH when clear.' },
    },
    props: {
      detected: 'Simulated detection state (0 = clear, 1 = obstacle detected).',
    },
    wiring: 'VCCâ†’5V, GNDâ†’GND, OUTâ†’D2.',
    code: `void setup(){
  pinMode(2, INPUT);
  pinMode(13, OUTPUT);
  Serial.begin(9600);
...
void loop(){
  ...
  flex_sensor: {
    ...
    icon: 'ã€°ï¸',
    category: 'Sensors',
    longDesc: 'A resistive flex sensor whose resistance increases when bent. Wired as a voltage divider, it produces a 0â€“1023 analog value proportional to the bend angle. Adjust the simulated bend value in the property panel.',
    ...
    pins: {
      ...
      p2: { label: '2', type: 'analog', desc: 'Analog output â€” connect to an analog input pin.' },
    },
    props: {
      bend: 'Simulated bend angle (0 = straight, 1023 = fully bent).',
    },
    wiring: '5Vâ†’p1 (via 10kÎ© resistor), p2â†’A0, other end of fixed resistorâ†’GND.',
    code: `void setup(){
  Serial.begin(9600);
}
void loop(){
  ...
  thermistor: {
    ...
    pins: {
      p1: { label: '1', type: 'analog', desc: 'Analog output â€” connect to an analog input pin.' },
      p2: { label: '2', type: 'gnd', desc: 'Connect to GND (through the thermistor to form a voltage divider).' },
    },
    props: {
      temperature: 'Simulated temperature in Â°C (default 25Â°C).',
    },
    wiring: '5Vâ†’10kÎ© fixed resistorâ†’p1, p1â†’A0, p2â†’GND.',
    code: `void setup(){
  Serial.begin(9600);
}
void loop(){
  ...
  diode_1n4007: {
    ...
    pins: {
      anode: { label: 'A', type: 'signal', desc: 'Anode â€” current flows INTO this pin (from positive side).' },
      cathode: { label: 'K', type: 'signal', desc: 'Cathode â€” current flows OUT of this pin (toward load/GND).' },
    },
    props: {},
    wiring: 'Place in series: anode to positive source, cathode to load.',
    code: `// Diodes are passive components â€” no code needed.
// Anode (+) -> Diode -> Cathode (-) -> Load`,
  ...
  l298n: {
    ...
    pins: {
      ...
      ENA: { label: 'ENA', type: 'pwm', desc: 'Motor A speed (PWM) â€” jumper on for full speed.' },
      ENB: { label: 'ENB', type: 'pwm', desc: 'Motor B speed (PWM) â€” jumper on for full speed.' },
      ...
      VS: { label: 'VS', type: 'power', desc: 'Motor supply voltage (6â€“12V). 5V pin output when jumper is on.' },
      GND: { label: 'GND', type: 'gnd', desc: 'Common ground â€” must share GND with Arduino.' },
    },
    props: {},
    wiring: 'IN1â†’D8, IN2â†’D9, ENAâ†’D10(PWM). VSâ†’external 7â€“12V. GNDâ†’Arduino GND. OUT1/OUT2â†’Motor A.',
    code: `void setup(){
  pinMode(8, OUTPUT);
  pinMode(9, OUTPUT);
  pinMode(10, OUTPUT);
...
void loop(){
  ...
  servo_continuous: {
    ...
    icon: 'âš™ï¸',
    category: 'Actuators',
    longDesc: 'Continuous rotation servo motor. Unlike standard servos (0â€“180Â°), this spins continuously in both directions. A 1500Âµs pulse stops the motor; above 1500Âµs spins one direction, below spins the other. Speed is proportional to pulse width deviation.',
    use: 'Wheels on robots, conveyor belts, panning mechanisms. Use analogWrite or Servo library to set speed.',
    pins: {
      signal: { label: 'SIG', type: 'pwm', desc: 'PWM signal â€” pulse width controls speed and direction.' },
      vcc: { label: '+', type: 'power', desc: 'Power supply (4.8â€“6V typical).' },
      gnd: { label: 'âˆ’', type: 'gnd', desc: 'Ground.' },
    },
    props: {
      speed: 'Simulated speed: âˆ’100% (full CCW) to +100% (full CW), 0% = stopped.',
    },
    wiring: 'SIGâ†’D9 (PWM), +â†’5V (or external 5V), âˆ’â†’GND.',
    code: `void setup(){
  pinMode(9, OUTPUT);
}
void loop(){
  ...
  rotary_encoder: {
    ...
    icon: 'ðŸŽ›ï¸',
    category: 'Input',
    longDesc: 'EC11 rotary encoder with push button. Provides infinite rotation with quadrature output (A, B channels 90Â° out of phase). Turns left/right and presses like a button. Use interrupts for accurate position tracking.',
    use: 'Menu navigation, volume control, parameter adjustment. Use attachInterrupt() on pin A to count edges.',
    pins: {
      A: { label: 'A', type: 'digital', desc: 'Channel A â€” connect to interrupt pin (D2/D3).' },
      B: { label: 'B', type: 'digital', desc: 'Channel B â€” connect to digital input.' },
      SW: { label: 'SW', type: 'digital', desc: 'Push switch â€” active LOW (internal pull-up).' },
      GND: { label: 'GND', type: 'gnd', desc: 'Ground.' },
    },
    props: {
      position: 'Rotational position (drag slider to simulate turning).',
    },
    wiring: 'Aâ†’D2 (interrupt), Bâ†’D3, SWâ†’D4 (INPUT_PULLUP), GNDâ†’GND.',
...
void loop(){
  ...
  dip_switch: {
    ...
    icon: 'ðŸŽšï¸',
    category: 'Input',
    longDesc: '8-position DIP switch bank. Each toggle is an independent SPST switch â€” up = ON (HIGH), down = OFF (LOW). Commonly used for hardware address configuration, mode selection, or binary input.',
    use: 'Set binary input values, hardware configuration, mode selection. Read each pin with digitalRead().',
    pins: {
      '1': { label: '1', type: 'digital', desc: 'Switch 1 â€” bit 0 of binary value.' },
      '2': { label: '2', type: 'digital', desc: 'Switch 2 â€” bit 1 of binary value.' },
      '3': { label: '3', type: 'digital', desc: 'Switch 3 â€” bit 2.' },
      '4': { label: '4', type: 'digital', desc: 'Switch 4 â€” bit 3.' },
      '5': { label: '5', type: 'digital', desc: 'Switch 5 â€” bit 4.' },
      '6': { label: '6', type: 'digital', desc: 'Switch 6 â€” bit 5.' },
      '7': { label: '7', type: 'digital', desc: 'Switch 7 â€” bit 6.' },
      '8': { label: '8', type: 'digital', desc: 'Switch 8 â€” bit 7 (MSB).' },
    },
    props: {
      switches: '8-bit binary value (0â€“255). Each bit controls one switch position.',
    },
    wiring: 'Connect pins 1â€“8 to Arduino digital inputs D2â€“D9. Use INPUT_PULLUP for each pin.',
...
void loop(){
  ...
  /* â”€â”€ SENSORS / COMMUNICATION â”€â”€ */
  hc05: {
    id: 'hc05',
    name: 'HC-05 Bluetooth',
    icon: 'ðŸ“¶',
    category: 'Sensors',
    longDesc: 'HC-05 serial-to-Bluetooth transceiver module (UART). Pairs with a phone/tablet to send and receive data over Bluetooth Serial Port Profile (SPP). Defaults to 9600 baud. TXDâ†’Arduino RX, RXDâ†’Arduino TX (with voltage divider for 3.3V logic).',
    use: 'Wireless communication with phone apps, remote control, telemetry. Send characters from phone to Arduino via Bluetooth.',
    pins: {
      VCC: { label: 'VCC', type: 'power', desc: 'Power supply (3.6â€“6V).' },
      GND: { label: 'GND', type: 'gnd', desc: 'Ground.' },
      TXD: { label: 'TXD', type: 'digital', desc: 'Bluetooth TX â†’ Arduino RX (D0).' },
      RXD: { label: 'RXD', type: 'digital', desc: 'Bluetooth RX â†’ Arduino TX (D1) via voltage divider.' },
    },
    props: {
      connected: 'Toggle Bluetooth connection status (simulated).',
    },
    wiring: 'VCCâ†’5V, GNDâ†’GND, TXDâ†’Arduino D0 (RX), RXDâ†’Arduino D1 (TX via 1kÎ©/2kÎ© divider).',
    code: `void setup(){
  Serial.begin(9600);
  pinMode(13, OUTPUT);
}
void loop(){
  ...
  ds3231: {
    ...
    icon: 'ðŸ•',
    category: 'Sensors',
    longDesc: 'DS3231 high-precision RTC (Real-Time Clock) module with battery backup â€” I2C @ 0x68. Provides year/month/day/hour/minute/second with Â±2ppm accuracy, built-in temperature sensor, and two programmable alarms. Time is simulated in real-time during simulation.',
    ...
    pins: {
      ...
      GND: { label: 'GND', type: 'gnd', desc: 'Ground.' },
      SCL: { label: 'SCL', type: 'digital', desc: 'I2C clock line â€” connect to A5 (Uno) or SCL.' },
      SDA: { label: 'SDA', type: 'digital', desc: 'I2C data line â€” connect to A4 (Uno) or SDA.' },
    },
    props: {
      hour: 'Initial hour (0â€“23).',
      minute: 'Initial minute (0â€“59).',
      second: 'Initial second (0â€“59).',
      day: 'Initial day (1â€“31).',
      month: 'Initial month (1â€“12).',
      year: 'Initial year (0â€“99, represents 2000â€“2099).',
      temperature: 'Simulated temperature in Â°C.',
    },
    wiring: 'VCCâ†’5V, GNDâ†’GND, SCLâ†’A5, SDAâ†’A4.',
    code: `#include <Wire.h>
#include <RtcDS3231.h>
...
void loop(){
  ...
  vl53l0x: {
    ...
    wiring: 'VCCâ†’3.3V/5V, GNDâ†’GND, SDAâ†’A4, SCLâ†’A5.',
    code: `#include <Wire.h>
...
void loop() {
  ...
  bme280: {
    ...
    props: {
      temperature: 'Simulated temperature in Celsius (default 25Â°C).',
...
void loop() {
  ...
  /* â”€â”€ LED COLOR VARIANTS â”€â”€ */
  led_green: {
    id: 'led_green', name: 'LED (Green)', icon: '\u{1f7e2}', category: 'Output',
    longDesc: 'Green light-emitting diode. Identical electrical characteristics to a standard LED â€” always pair with a 220\u03a9 series resistor.',
  ...
  speaker_4ohm: {
    ...
    code: '// Speaker needs an amplifier â€” see MAX98357A example.',
  },
  ...
  water_flow_sensor: {
    ...
    pins: { VCC: { label: 'VCC', type: 'power', desc: '+5 V supply (red wire).' }, SIG: { label: 'SIG', type: 'digital', desc: 'Pulse output â€” connect to an Arduino interrupt-capable digital pin (e.g. D2).' }, GND: { label: 'GND', type: 'gnd', desc: 'Common ground (black wire).' } },
  ...
  breadboard_small: {
    ...
    code: '// Breadboard is passive â€” no code needed.',
  },
  ...
  mb102_power: {
    ...
    code: '// Power module is passive â€” no code needed.',
  },
  ...
  wifi_module: {
    ...
    wiring: 'No physical wiring â€” this is a wireless access point. Configure SSID and password in the properties panel, then use WiFi.begin(ssid, password) in your Arduino code.',
  ...
  ic_74hc245: {
    ...
    code: '// Bus transceiver is combinational â€” set DIR pin HIGH or LOW.',
  },
  ...
  ic_74hc47: {
    ...
    code: '// Connect BCD counter outputs to A\u2013D. Segments drive directly â€” no code needed.',
  },
  ...
  lm741: {
    ...
    code: '// Op-amp is analog â€” configure gain with external R1, R2 resistors.\n// Non-inverting: Vout = Vin \u00d7 (1 + R2/R1)',
  },
  ...
  dso_4ch: {
    ...
    code: '// Oscilloscope is a measurement instrument â€” no Arduino code needed.',
  },
  ...
  bh1750: {
    ...
    icon: 'â˜€',
    category: 'Sensors',
    longDesc: 'BH1750 ambient light intensity sensor module using I2C communication at address 0x23. Digital output in lux (1â€“65535 lx). Used in automatic lighting systems, weather stations, and light-sensitive projects.',
    use: 'Automatic lighting, brightness control, environmental monitoring.',
    pins: {
      vcc: { label: 'VCC', type: 'power', desc: '3.3Vâ€“5V power supply.' },
      gnd: { label: 'GND', type: 'gnd', desc: 'Ground.' },
      scl: { label: 'SCL', type: 'digital', desc: 'I2C clock â€” connect to A5 (Uno) / D22 (ESP32).' },
      sda: { label: 'SDA', type: 'digital', desc: 'I2C data â€” connect to A4 (Uno) / D21 (ESP32).' },
    },
  ...
  ina219: {
    ...
    icon: 'âš¡',
    category: 'Sensors',
    longDesc: 'INA219 bidirectional current and power monitor module using I2C at address 0x40. Measures bus voltage (0â€“26V), shunt voltage, current (up to 3.2A), and power.',
  ...
  max6675: {
    ...
    icon: 'ðŸ”¥',
    category: 'Sensors',
    longDesc: 'MAX6675 K-type thermocouple-to-digital converter using SPI. Measures 0â€“1024 Â°C with 0.25 Â°C resolution.',
    use: 'High-temperature measurement, oven control, industrial sensing.',
    pins: {
      vcc: { label: 'VCC', type: 'power', desc: '5V power supply.' },
      gnd: { label: 'GND', type: 'gnd', desc: 'Ground.' },
      cs: { label: 'CS', type: 'digital', desc: 'Chip select â€” any digital pin.' },
      clk: { label: 'CLK', type: 'digital', desc: 'SPI clock â€” D13 (Uno).' },
      do: { label: 'DO', type: 'digital', desc: 'SPI data out â€” D12 (Uno).' },
    },
    wiring: 'VCCâ†’5V, GNDâ†’GND, CSâ†’D10, CLKâ†’D13, DOâ†’D12.',
  ...
  tft_display: {
    ...
    pins: {
      ...
      gnd: { label: 'GND', type: 'gnd', desc: 'Ground.' },
      sck: { label: 'SCK', type: 'digital', desc: 'SPI clock â€” D13 (Uno).' },
      mosi: { label: 'MOSI', type: 'digital', desc: 'SPI data â€” D11 (Uno).' },
      cs: { label: 'CS', type: 'digital', desc: 'Chip select â€” D10 (Uno).' },
    },
    wiring: 'VCCâ†’3.3V, GNDâ†’GND, SCKâ†’D13, MOSIâ†’D11, CSâ†’D10.',
  ...
  sd_card: {
    ...
    pins: {
      vcc: { label: 'VCC', type: 'power', desc: '5V power supply.' },
      gnd: { label: 'GND', type: 'gnd', desc: 'Ground.' },
      miso: { label: 'MISO', type: 'digital', desc: 'SPI master in slave out â€” D12 (Uno).' },
      mosi: { label: 'MOSI', type: 'digital', desc: 'SPI master out slave in â€” D11 (Uno).' },
      sck: { label: 'SCK', type: 'digital', desc: 'SPI clock â€” D13 (Uno).' },
      cs: { label: 'CS', type: 'digital', desc: 'Chip select â€” D10 (Uno).' },
    },
    wiring: 'VCCâ†’5V, GNDâ†’GND, CSâ†’D10, MOSIâ†’D11, MISOâ†’D12, SCKâ†’D13.',
  ...
  ir_led: {
    ...
    icon: '\uD83D\uDD34',
    category: 'Sensors',
    longDesc: 'Infrared LED transmitter â€” emits IR light when driven HIGH. Use with a current-limiting resistor (220\u03A9). Pair with an IR Receiver TSOP4838 for wireless communication.',
    use: 'IR remote control transmitters, obstacle detection, proximity sensors.',
    pins: {
      anode: { label: '+', type: 'digital', desc: 'Anode â€” connect to a digital pin through a 220\u03A9 resistor.' },
      cathode: { label: '\u2212', type: 'gnd', desc: 'Cathode â€” connect to GND.' },
    },
  ...
  ir_remote: {
    ...
    icon: '\uD83C\uDFAE',
    category: 'Input',
    longDesc: 'Battery-powered infrared remote control with 19 buttons (CH\u2212/CH/CH+, PREV/NEXT/PLAY, \u2212/+/EQ, 0\u20139). Click any button to send its NEC IR code to all IR Receiver components on the canvas. No wiring needed \u2014 uses internal CR2025 battery.',
    use: 'Wireless control of Arduino projects \u2014 navigate menus, control servos, toggle LEDs.',
    pins: {},
    wiring: 'No wiring needed. Pair with IR Receiver TSOP4838 (OUT\u2192D2) and IR LED on the Arduino.',
...
NewPing sonar(TRIGGER_PIN, ECHO_PIN, MAX_DISTANCE);
  ...
  /* â”€â”€ ACTUATORS â”€â”€ */
  {
    ...
    category: 'Actuators',
    include: '<Servo.h>',
    desc: 'Control servo motors (SG90, MG996R, etc.) via PWM. Supports angles from 0Â° to 180Â°.',
    api: [
      { fn: 'servo.attach(pin)', desc: 'Attach servo to pin' },
      { fn: 'servo.write(angle)', desc: 'Set angle (0-180Â°)' },
      { fn: 'servo.writeMicroseconds(us)', desc: 'Set pulse width (500-2400Î¼s)' },
...
const GUIDE_TUTORIALS = [
  {
    ...
    steps: [
      ...
      'Every new project opens with the classic Blink example â€” an Arduino Uno with an LED on pin 13.',
      ...
      'Press F6 to stop, F7 to pause/resume. Drag the "Speed" selector to run the simulation up to 10Ã— faster.',
    ],
    wiring: 'No wiring required â€” the built-in Blink circuit is already set up for you.',
    code: `void setup() {
    pinMode(13, OUTPUT);
    Serial.begin(9600);
  }
...

  {
    ...
    summary: 'The classic "Hello World" â€” turn an LED on and off at a fixed interval.',
    steps: [
      'From the component library drag an LED and a Resistor onto the canvas, or keep the default circuit.',
      'Wire D13 â†’ resistor â†’ LED anode, then LED cathode â†’ GND. Click a pin, then click the next pin to draw a wire.',
    ...
    wiring: 'D13 â†’ 220 Î© resistor â†’ LED anode(+) ; LED cathode(âˆ’) â†’ GND.',
    code: `void setup() {
    pinMode(13, OUTPUT);
  }
  void loop() {
...

  {
    ...
    wiring: 'D9 â†’ resistor â†’ LED anode(+) ; LED cathode(âˆ’) â†’ GND.',
    code: `int ledPin = 9;
int brightness = 0;
int fadeAmount = 5;
  void setup() {
...

  {
    ...
    steps: [
      'Place a push button. Wire D2 â†’ pin 1 and pin 3 â†’ GND. Wire the LED to D13 as in the Blink tutorial.',
      'Set pin 2 to INPUT_PULLUP â€” the pin reads HIGH when released, LOW when pressed.',
    ...
    wiring: 'D2 â†’ button pin 1 ; button pin 3 â†’ GND ; D13 â†’ LED.',
...
int ledPin = 13;
...

  {
    ...
    summary: 'Read a knob position as a 0â€“1023 value and print it to the Serial Monitor.',
    steps: [
      'Wire the potentiometer: VCCâ†’5V, OUTâ†’A0, GNDâ†’GND.',
      ...
      'Tip: change the value in the property panel (right-click â†’ Properties) for precise control.',
    ],
    wiring: 'VCCâ†’5V, OUTâ†’A0, GNDâ†’GND.',
    code: `void setup() {
    Serial.begin(9600);
...

  {
    ...
    summary: 'Sweep a servo between 0Â° and 180Â° using the Servo library.',
    steps: [
      'Place a servo. Wire SIGâ†’D9, +â†’5V, âˆ’â†’GND.',
...

  {
    ...
    summary: 'Show text and live values on a 16Ã—2 I2C LCD using just four wires.',
    steps: [
      'Place an LCD 16Ã—2 (I2C). Wire VCCâ†’5V, GNDâ†’GND, SDAâ†’A4, SCLâ†’A5.',
    ...
    wiring: 'VCCâ†’5V, GNDâ†’GND, SDAâ†’A4 (Uno), SCLâ†’A5 (Uno).',
    code: `#include < LiquidCrystal_I2C.h >
    LiquidCrystal_I2C lcd(0x27, 16, 2);
  void setup() {
    lcd.init();
...

  {
    ...
    steps: [
      'Place a DHT11. Wire VCCâ†’5V, DATâ†’D2, GNDâ†’GND.',
      ...
      'Adjust the simulated values in the property panel (right-click the sensor â†’ Properties).',
    ],
    wiring: 'VCCâ†’5V, DATâ†’D2, GNDâ†’GND.',
    code: `#include < DHT.h >
    DHT dht(2, DHT11);
...

  {
    ...
    steps: [
      'Place an HC-SR04. Wire VCCâ†’5V, TRIGâ†’D7, ECHOâ†’D8, GNDâ†’GND.',
      'Set TRIG as OUTPUT and ECHO as INPUT.',
      'Send a 10 Âµs HIGH pulse on TRIG, then pulseIn(ECHO, HIGH).',
...

  {
    ...
    steps: [
      'Write code that Serial.print()s numbers â€” one per line, or multiple values separated by spaces or tabs.',
    ...
    wiring: 'Potentiometer optional â€” this works with any numeric serial output.',
    code: `void setup() {
    Serial.begin(9600);
  }
  void loop() {
...

  {
    ...
    steps: [
      'Place an RGB LED. Wire Râ†’D9, Gâ†’D10, Bâ†’D11, common(âˆ’)â†’GND.',
    ...
    wiring: 'Râ†’D9, Gâ†’D10, Bâ†’D11, âˆ’â†’GND.',
    code: `void setup() {
    pinMode(9, OUTPUT); pinMode(10, OUTPUT); pinMode(11, OUTPUT);
  }
  void loop() {
...

  {
    ...
    steps: [
      'Select a push button on the canvas and press Space or Enter to toggle it â€” no need to click.',
      'Press R to rotate the selected component by 90Â°.',
    ...
    wiring: 'N/A â€” these are editor/canvas shortcuts.',
...
const GUIDE_HOME = {
  tagline: 'Simulate Arduino circuits in your browser â€” no install, no hardware.',
  intro: 'ArduSim is a full Arduino development environment that runs entirely in your browser. Write Arduino C++ in a VS Code-grade editor, drag components onto a live circuit canvas, wire them up, and hit Run â€” the simulation, Serial Monitor, oscilloscope and plotter all respond instantly.',
  features: [
    { icon: 'âš¡', title: 'Real-time Simulation', desc: 'Arduino code transpiles and runs in the browser at 0.25Ã—â€“10Ã— speed with infinite-loop protection.' },
    { icon: 'âŒ¨ï¸', title: 'VS Code-grade Editor', desc: 'Monaco-powered editor with autocomplete, syntax highlighting, diagnostics and formatting.' },
    { icon: 'ðŸ”§', title: 'Drag & Drop Circuitry', desc: '25+ components, click-to-connect wiring, grid snapping, rotate/duplicate/undo and PNG export.' },
    { icon: 'ðŸ“Ÿ', title: 'Serial Monitor & Plotter', desc: 'Live bi-directional serial I/O, multi-channel oscilloscope and a real-time data plotter.' },
    { icon: 'ðŸ“Š', title: 'Pin Monitor', desc: 'A live colour-coded board showing the state and mode of every pin.' },
    { icon: 'ðŸ’¾', title: 'Project Management', desc: 'Auto-save, portable JSON export/import, saved projects and one-click URL sharing.' },
  ],
  steps: [
    ...
    { title: 'Write the code', desc: 'Write or paste Arduino C++ in the editor â€” autocomplete and IntelliSense guide you.' },
  ...
  cta: 'View Full Guide â†’',
};

/* â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
   GUIDE MANAGER â€” renders Home / Components / Tutorials
   â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â• */
...
class GuideManager {
  ...
  /* â”€â”€ bind header tabs / close (called when the app is ready) â”€â”€ */
  bind() {
  ...
  _renderHome() {
    ...
    root.innerHTML = `
      <div class="guide-hero">
        <div class="guide-hero-badge">100% in-browser Â· No installation</div>
        <div class="guide-hero-icon">ðŸŽ›ï¸</div>
        ...
        <div class="guide-hero-cta">
          <button class="gh-btn gh-btn-primary" id="gh-go-sim">âš¡ Launch the Simulator</button>
      ...
      <div class="gh-cta-band">
        ...
        <button class="gh-btn gh-btn-primary" id="gh-go-sim2">Start Simulating â†’</button>
      </div>

      <footer class="guide-footer">ArduSim Â· Free, open-source Arduino simulator for education and hobbyists.</footer>`;
    ...
    root.innerHTML = `
      <div class="gc-toolbar">
        <div class="library-search">
          ...
          <input type="text" id="gc-search" placeholder="Search componentsâ€¦" aria-label="Search components" />
        </div>
      ...
      return `
        <button class="gc-card" data-comp="${this._esc(id)}">
          <div class="gc-card-head">
            <span class="gc-card-icon">${def.icon || 'ðŸ”§'}</span>
            <div>
  ...
  _renderCompDetail(root, id) {
    ...
    const pinRows = grouped ? (() => {
      // Grouped pins (boards) â€” document by function group
    ...
    const propsBlock = Object.keys(g.props || {}).length ? `
      <div class="gc-detail-section">
        ...
        <ul class="gc-props">
          ${Object.entries(g.props).map(([k, v]) => `<li><code>${this._esc(k)}</code> â€” ${this._esc(v)}</li>`).join('')}
        </ul>
      </div>` : '';
    ...
    root.innerHTML = `
      <button class="gh-btn gh-btn-ghost gh-btn-sm gc-back">â† Back to all components</button>
      <div class="gc-detail">
        <div class="gc-detail-head">
          <span class="gc-card-icon gc-card-icon-lg">${def.icon || 'ðŸ”§'}</span>
          <div>
        ...
        <div class="gc-detail-section">
          <h3>Typical use</h3>
          <p class="gc-long-desc">${this._esc(g.use || 'â€”')}</p>
        </div>
        ...
        <div class="gc-detail-section">
          ...
          <p class="gc-wiring">${this._esc(g.wiring || 'â€”')}</p>
        </div>
  ...
  _renderLibraryDetail(root, id) {
    ...
    root.innerHTML = `
      <button class="gh-btn gh-btn-ghost gh-btn-sm gc-back">â† All libraries</button>
      <div class="gc-detail">
        <div class="gc-detail-head">
  ...
  _renderTutorialDetail(root, id) {
    ...
    root.innerHTML = `
      <button class="gh-btn gh-btn-ghost gh-btn-sm gc-back">â† All tutorials</button>
      <div class="gc-detail">
        <div class="gc-detail-head">
        ...
        <div class="gc-detail-section">
          <h3>Wiring</h3>
          <p class="gc-wiring">${this._esc(t.wiring || 'â€”')}</p>
        </div>
  ...
  /* â”€â”€ actions that talk to the app â”€â”€ */
  _placeComponent(id) {
    ...
    if (app && app.canvas && app.canvas.startPlacing) {
      ...
      const def = (window.ArduinoComponents && window.ArduinoComponents.COMPONENT_DEFS[id]);
      app.showToast(`${def ? def.name : id} selected â€” click on canvas to place`, 'info');
    }
  }
