...
class App {
  ...
  _attachSim2Events() {
    ...
    this.sim2.onEvent = (type, data) => {
      ...
      this._handleOledEvents(insts, type, data);
    };
  }

  _handleOledEvents(insts, type, data) {
    if (!this.canvas) return;
    for (const inst of insts) {
...
void loop() {
  ...
  saveProject() {
    const files = this.editor?.getAllFiles?.() || { [this._getDefaultSketchName()]: '' };
    const board2Code = this._getBoard2Code();
  ...
  loadProject() {
    window.StorageManager?.loadFromFile((project) => {
      if (this.editor && project.files) {
        ...
        this.editor.loadFiles({ [this._getDefaultSketchName()]: project.code });
      }
  ...
  _loadSavedProject(id) {
    ...
    if (this.editor && p.files) {
      ...
      this.editor.loadFiles({ [this._getDefaultSketchName()]: p.code });
    }
  ...
  _downloadSavedProject(id) {
    const p = (window.StorageManager?.getSavedProjects() || []).find(x => x.id === id);
    if (!p) return;
    const files = p.files || (p.code ? { [this._getDefaultSketchName()]: p.code } : { [this._getDefaultSketchName()]: '' });
...
_newProject() {
  ...
  _triggerAutoSave() {
    ...
    const files = this.editor?.getAllFiles?.() || { [this._getDefaultSketchName()]: this.editor?.getCode?.() || '' };
    const board2Code = this._getBoard2Code();
  ...
  _restoreProject() {
    ...
    if (shared) {
      if (this.editor && shared.files) {
        ...
        this.editor.loadFiles({ [this._getDefaultSketchName()]: shared.code });
      }
  ...
  _buildMemoryViewer() {
    ...
    if (this._memoryView === undefined) this._memoryView = 'ram';
    if (this._xramPageAddr === undefined) this._xramPageAddr = 0;
    const sel = document.getElementById('memory-view-select');
    if (sel && !sel._xramBound) {
      sel._xramBound = true;
      sel.value = this._memoryView;
      sel.addEventListener('change', (e) => {
  ...
  _attachXramEditHandlers() {
    const viewer = document.getElementById('memory-viewer');
    if (!viewer || viewer._xramEditBound) return;
    viewer._xramEditBound = true;
    viewer.addEventListener('dblclick', (e) => {
      ...
      input.addEventListener('keydown', (ev) => { if (ev.key === 'Enter') commit(); if (ev.key === 'Escape') { committed = true; cell.textContent = curVal; input.remove(); } });
  ...
  _updateMemory() {
    ...
    const viewer = document.getElementById('memory-viewer');
    if (!viewer) return;
    if (!viewer.querySelector('.mem-byte')) {
      this._buildMemoryViewer();
    }
    if (this._memoryView === 'xram' && is8051) {
      if (this.sim && this.sim.isRunning) this._applyXramData();
      const base = this._xramPageAddr || 0;
      let bytes = null;
      ...
      for (let i = 0; i < bytes.length; i++) {
        ...
        const el = viewer.querySelector(`.mem-byte[data-addr="${absAddr}"]`);
        if (!el) continue;
    ...
    for (let i = start; i < end; i++) {
      ...
      item.addEventListener('click', () => {
        ...
        if (this.editor && example.files) {
          ...
          this.editor.loadFiles({ [this._getDefaultSketchName()]: example.code || '' });
        }
  ...
  loadExampleById(id) {
    ...
    if (this.editor && ex.files) {
      ...
      this.editor.loadFiles({ [this._getDefaultSketchName()]: ex.code || '' });
    }
