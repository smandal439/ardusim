...
//     // ── PCB Body (STM32 signature blue) ──
//     const grad = ctx.createLinearGradient(0, 0, W, H);
//     grad.addColorStop(0, '#1a3a6a');
//     grad.addColorStop(0.3, '#1e4478');
//     grad.addColorStop(0.7, '#1a3a6a');
//     grad.addColorStop(1, '#122a50');
//     ctx.fillStyle = grad;
//     roundRect(ctx, 0, 0, W, H, 8);
//     ctx.fill();
...
//         ctx.shadowBlur = 3;
//         ctx.beginPath();
//         ctx.arc(holeX, hy, 3.5, 0, Math.PI * 2);
//         ctx.fill();
//         ctx.shadowBlur = 0;
//         ctx.fillStyle = '#0a0a0c';
//         ctx.beginPath();
...
//     drawVertHeader(W - 18, rightYs, W - 11);
...
//       let bgColor = 'rgba(10,20,40,0.85)';
//       if (pin.type === PIN_TYPE.POWER) bgColor = 'rgba(180,40,40,0.8)';
//       else if (pin.type === PIN_TYPE.GND) bgColor = 'rgba(40,40,40,0.8)';
//       else if (pin.type === PIN_TYPE.ANALOG) bgColor = 'rgba(30,80,120,0.8)';
//       else if (pin.type === PIN_TYPE.PWM) bgColor = 'rgba(120,80,30,0.8)';
...
defComp({
  ...
  pins: [
    // ── Left Arduino connector (vertical) ──
    { id: 'A0',   label: 'A0',         type: PIN_TYPE.ANALOG,  x: 18, y: 35,  side: 'left' },
    { id: 'A1',   label: 'A1',         type: PIN_TYPE.ANALOG,  x: 18, y: 54,  side: 'left' },
    { id: 'A2',   label: 'A2',         type: PIN_TYPE.ANALOG,  x: 18, y: 73,  side: 'left' },
    { id: 'A3',   label: 'A3',         type: PIN_TYPE.ANALOG,  x: 18, y: 92,  side: 'left' },
    { id: 'A4',   label: 'A4',         type: PIN_TYPE.ANALOG,  x: 18, y: 111, side: 'left' },
    { id: 'A5',   label: 'A5',         type: PIN_TYPE.ANALOG,  x: 18, y: 130, side: 'left' },
    { id: 'D0',   label: 'D0 (RX)',    type: PIN_TYPE.SIGNAL,  x: 18, y: 154, side: 'left' },
    { id: 'D1',   label: 'D1 (TX)',    type: PIN_TYPE.SIGNAL,  x: 18, y: 173, side: 'left' },
    { id: 'D2',   label: 'D2',         type: PIN_TYPE.DIGITAL, x: 18, y: 192, side: 'left' },
    { id: 'D3',   label: 'D3~',        type: PIN_TYPE.PWM,     x: 18, y: 211, side: 'left' },
    { id: 'D4',   label: 'D4',         type: PIN_TYPE.DIGITAL, x: 18, y: 230, side: 'left' },
    { id: 'D5',   label: 'D5~',        type: PIN_TYPE.PWM,     x: 18, y: 249, side: 'left' },
    { id: 'D6',   label: 'D6~',        type: PIN_TYPE.PWM,     x: 18, y: 268, side: 'left' },
    { id: 'GND1', label: 'GND',        type: PIN_TYPE.GND,     x: 18, y: 287, side: 'left' },

    // ── Right Arduino connector (vertical) ──
    { id: 'D7',   label: 'D7',         type: PIN_TYPE.DIGITAL, x: 462, y: 35,  side: 'right' },
    { id: 'D8',   label: 'D8',         type: PIN_TYPE.DIGITAL, x: 462, y: 54,  side: 'right' },
    { id: 'D9',   label: 'D9~',        type: PIN_TYPE.PWM,     x: 462, y: 73,  side: 'right' },
    { id: 'D10',  label: 'D10~',       type: PIN_TYPE.PWM,     x: 462, y: 92,  side: 'right' },
    { id: 'D11',  label: 'D11~',       type: PIN_TYPE.PWM,     x: 462, y: 111, side: 'right' },
    { id: 'D12',  label: 'D12',        type: PIN_TYPE.DIGITAL, x: 462, y: 130, side: 'right' },
    { id: 'D13',  label: 'D13 · L',    type: PIN_TYPE.DIGITAL, x: 462, y: 154, side: 'right' },
    { id: 'D14',  label: 'D14 (SDA)',  type: PIN_TYPE.SIGNAL,  x: 462, y: 179, side: 'right' },
    { id: 'D15',  label: 'D15 (SCL)',  type: PIN_TYPE.SIGNAL,  x: 462, y: 198, side: 'right' },
    { id: 'VIN',  label: 'VIN',        type: PIN_TYPE.POWER,   x: 462, y: 223, side: 'right' },
    { id: '5V',   label: '5V',         type: PIN_TYPE.POWER,   x: 462, y: 242, side: 'right' },
    { id: '3V3',  label: '3.3V',       type: PIN_TYPE.POWER,   x: 462, y: 261, side: 'right' },
    { id: 'GND2', label: 'GND',        type: PIN_TYPE.GND,     x: 462, y: 287, side: 'right' },

    // ── Bottom edge pins ──
    { id: 'NRST', label: 'NRST',       type: PIN_TYPE.SIGNAL,  x: 90,  y: 310, side: 'bottom' },
    { id: 'PB6',  label: 'PB6',        type: PIN_TYPE.SIGNAL,  x: 150, y: 310, side: 'bottom' },
    { id: 'PB7',  label: 'PB7',        type: PIN_TYPE.SIGNAL,  x: 210, y: 310, side: 'bottom' },
    { id: 'PA9',  label: 'PA9',        type: PIN_TYPE.SIGNAL,  x: 270, y: 310, side: 'bottom' },
    { id: 'PA10', label: 'PA10',       type: PIN_TYPE.SIGNAL,  x: 330, y: 310, side: 'bottom' },
    { id: 'PI11', label: 'PI11',       type: PIN_TYPE.SIGNAL,  x: 390, y: 310, side: 'bottom' },
  ],
  ...
  draw(ctx, inst, sim) {
    ...
    // Expanded LCD display screen (4.3" ratio scaled up)
    const LCD_W = 320, LCD_H = 175;
    const LCD_X = (W - LCD_W) / 2;
    const LCD_Y = 24;
    ...
    // ── PCB Body (Dark Blue ST Signature Finish) ──
    const grad = ctx.createLinearGradient(0, 0, W, H);
    grad.addColorStop(0, '#15315b');
    grad.addColorStop(0.35, '#1b4175');
    grad.addColorStop(0.7, '#15315b');
    grad.addColorStop(1, '#0e2140');
    ctx.fillStyle = grad;
    ...
    const holes = [[14, 14], [W - 14, 14], [14, H - 14], [W - 14, H - 14]];
    ...
    // Silkscreen Outer Margin Line
    ctx.strokeStyle = 'rgba(255,255,255,0.14)';
    ctx.lineWidth = 1;
    roundRect(ctx, 6, 6, W - 12, H - 12, 8);
    ctx.stroke();

    // ── LCD TFT Display Assembly ──
    // Metallic LCD Frame Bezel
    ctx.fillStyle = '#22262c';
    roundRect(ctx, LCD_X - 5, LCD_Y - 5, LCD_W + 10, LCD_H + 10, 6);
    ctx.fill();
    ctx.strokeStyle = '#3a404a';
    ctx.lineWidth = 1;
    ctx.stroke();

    // Glass Screen Dark Background
    ...
    ctx.lineWidth = 1.5;
    roundRect(ctx, LCD_X + 1, LCD_Y + 1, LCD_W - 2, LCD_H - 2, 2);
    ctx.stroke();

    // ── TFT Framebuffer / Idle Display Content ──
    const tftBuf = inst.runtimeState && inst.runtimeState.tft;
    if (tftBuf && tftBuf.pixels) {
      const FB_W = 480, FB_H = 272;
      const scaleX = (LCD_W - 4) / FB_W, scaleY = (LCD_H - 4) / FB_H;
      const px = tftBuf.pixels;
      const screenX = LCD_X + 2, screenY = LCD_Y + 2;
      ...
      // Sleek idle splash UI
      ctx.fillStyle = '#0e2b18';
      roundRect(ctx, LCD_X + 4, LCD_Y + 4, LCD_W - 8, LCD_H - 8, 2);
      ctx.fill();

      ctx.fillStyle = '#30d158';
      ctx.font = 'bold 13px monospace';
      ctx.textAlign = 'center';
      ctx.fillText('STM32F746G-DISCO', LCD_X + LCD_W / 2, LCD_Y + LCD_H / 2 - 20);

      ctx.fillStyle = '#1c8e3b';
      ctx.font = '10px monospace';
      ctx.fillText('480x272 TFT LCD Display', LCD_X + LCD_W / 2, LCD_Y + LCD_H / 2);
      ctx.fillText('ARM Cortex-M7 @ 216 MHz', LCD_X + LCD_W / 2, LCD_Y + LCD_H / 2 + 16);

      ctx.fillStyle = '#115824';
      ctx.font = '9px monospace';
      ctx.fillText('ILI9341 / RK043FN48H Controller Active', LCD_X + LCD_W / 2, LCD_Y + LCD_H / 2 + 38);
    }

    // Subtle Glass Glint Reflective Line
    const glint = ctx.createLinearGradient(LCD_X, LCD_Y, LCD_X + LCD_W, LCD_Y + LCD_H);
    ...
    // ── On-board Hardware Components ──

    // 1. ST-LINK Programmer Sub-system (Bottom Left)
    ctx.fillStyle = '#0c1626';
    roundRect(ctx, 50, H - 42, 85, 26, 4);
    ctx.fill();
    ctx.strokeStyle = 'rgba(255,255,255,0.1)';
    ctx.stroke();

    ctx.fillStyle = '#648ab3';
    ctx.font = 'bold 7px monospace';
    ctx.textAlign = 'center';
    ctx.fillText('ST-LINK/V2-1', 92, H - 26);

    // Mini-USB ST-LINK Connector
    ctx.fillStyle = '#95a5a6';
    roundRect(ctx, 68, H - 12, 48, 10, 3);
    ctx.fill();
    ctx.fillStyle = '#2c3e50';
    roundRect(ctx, 74, H - 9, 36, 6, 1);
    ctx.fill();

    // 2. USB OTG HS Port (Bottom Right)
    ctx.fillStyle = '#95a5a6';
    roundRect(ctx, W - 120, H - 12, 52, 10, 3);
    ctx.fill();
    ctx.fillStyle = '#2c3e50';
    roundRect(ctx, W - 114, H - 9, 40, 6, 1);
    ctx.fill();
    ctx.fillStyle = '#abb2bf';
    ctx.font = 'bold 6px monospace';
    ctx.textAlign = 'center';
    ctx.fillText('USB OTG HS', W - 94, H - 16);

    // 3. Ethernet RJ45 Jack (Top Left)
    ctx.fillStyle = '#4a5361';
    roundRect(ctx, 54, 18, 22, 18, 2);
    ctx.fill();
    ctx.fillStyle = '#1c222b';
    roundRect(ctx, 57, 21, 16, 12, 1);
    ctx.fill();

    // 4. MicroSD Card Connector (Top Right)
    ctx.fillStyle = '#3a3f47';
    roundRect(ctx, W - 76, 18, 22, 16, 2);
    ctx.fill();
    ctx.fillStyle = '#d0b060';
    ctx.fillRect(W - 72, 30, 14, 2);

    // 5. User Button (B1 - Blue)
    ctx.fillStyle = '#1b1e24';
    roundRect(ctx, 210, H - 32, 24, 15, 3);
    ctx.fill();
    ctx.fillStyle = '#0066cc';
    roundRect(ctx, 213, H - 29, 18, 9, 2);
    ctx.fill();
    ctx.fillStyle = 'rgba(255,255,255,0.7)';
    ctx.font = 'bold 6px monospace';
    ctx.textAlign = 'center';
    ctx.fillText('USER (B1)', 222, H - 35);

    // 6. Reset Button (B2 - Black/Red)
    ctx.fillStyle = '#1b1e24';
    roundRect(ctx, 246, H - 32, 24, 15, 3);
    ctx.fill();
    ctx.fillStyle = '#cc2222';
    roundRect(ctx, 249, H - 29, 18, 9, 2);
    ctx.fill();
    ctx.fillStyle = 'rgba(255,255,255,0.7)';
    ctx.font = 'bold 6px monospace';
    ctx.fillText('RST (B2)', 258, H - 35);

    // 7. Onboard LED LD1 (Connected to PI1 / D13)
    const ledCx = W - 42, ledCy = 210;
    const lit = sim && sim.pinStates && (sim.pinStates.pin_13 || 0) > 0;
    drawLED_on_board(ctx, ledCx, ledCy, lit ? '#00ff55' : '#1b4d24', 5);
    ctx.fillStyle = '#abb2bf';
    ctx.font = 'bold 6px monospace';
    ctx.textAlign = 'center';
    ctx.fillText('LD1', ledCx, ledCy + 11);
    ctx.fillStyle = 'rgba(255,255,255,0.4)';
    ctx.font = '5px monospace';
    ctx.fillText('PI1', ledCx, ledCy - 8);

    // ── Connector Pin Header Strips ──
    const pinsList = inst.pins || this.pins || [];
    const leftYs = pinsList.filter(p => p.side === 'left').map(p => p.y);
    const rightYs = pinsList.filter(p => p.side === 'right').map(p => p.y);
    ...
    const drawVertHeader = (hx, holeYs, holeX) => {
      ...
      const topY = Math.min(...holeYs) - 10;
      const botY = Math.max(...holeYs) + 10;
    ...
    for (const pin of pinsList) {
      ...
      const bx = pin.side === 'left' ? pin.x - 2 - pw : pin.x + 2;

      let bgColor = 'rgba(12,24,44,0.9)';
    ...
    // ── Board Silkscreen Branding Text ──
    ctx.fillStyle = 'rgba(255,255,255,0.2)';
    ctx.font = 'bold 7px monospace';
    ctx.textAlign = 'center';
    ctx.fillText('STM32F746NGH6 · Cortex-M7 · 1MB Flash · 340KB SRAM', W / 2, H - 4);
