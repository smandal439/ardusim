...
const pathRules = [
  ...
  '    [/([A-Za-z_$][\\w$]*(?:\\s*\\[\\s*(["\\'])[^"\\\']+\\1\\s*\\])+\\s*)\\.as\\s*(?:<[^>]*>)?\\s*\\(\\s*\\)/g, function(m, expr) {',
  '      const keys = [];',
  '      const re = /\\[\\s*(["\\'])[^"\\\']+\\1\\s*\\]/g;',
  '      let k;',
  '      while ((k = re.exec(expr))) keys.push("\'" + k[2].replace(/\'/g, "\\\\'") + "\'");',
  '      const obj = expr.replace(/\\s*\\[\\s*(["\\'])[^"\\\']+\\1\\s*\\]/g, \'\');',
  '      return obj + "._asPath(" + keys.join(",") + ")";',
  '    }],',
  '    // doc["key"].is<T>() → doc._isPath(\'key\')  (run BEFORE bare .is rule)',
  '    [/([A-Za-z_$][\\w$]*(?:\\s*\\[\\s*(["\\'])[^"\\\']+\\2\\s*\\])+\\s*)\\.is\\s*(?:<[^>]*>)?\\s*\\(\\s*\\)/g, function(m, expr) {',
  '      const keys = [];',
  '      const re = /\\[\\s*(["\\'])[^"\\\']+\\1\\s*\\]/g;',
  '      let k;',
  '      while ((k = re.exec(expr))) keys.push("\'" + k[2].replace(/\'/g, "\\\\'") + "\'");',
